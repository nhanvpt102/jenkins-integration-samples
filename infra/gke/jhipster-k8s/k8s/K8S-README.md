...
```
$ docker image tag gateway gcr.io/gateway
$ docker push gcr.io/gateway
```
...
```
$ kubectl scale deployment <app-name> --replicas <replica-count> -n infra
```
...
```
$ kubectl set image deployment/<app-name>-app <app-name>=<new-image>  -n infra
```
...
```
$ kubectl expose service jhipster-registry --type=NodePort --name=exposed-registry -n infra
```

and explore the details using

```
$ kubectl get svc exposed-registry -n infra
```

For scaling the JHipster registry, use

```
$ kubectl scale statefulset jhipster-registry --replicas 3 -n infra
```
